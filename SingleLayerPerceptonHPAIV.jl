...
function get_data(MSA, AA_dict, cross, cv=10)
    ...
    test_data = [(data_encoded[i,:], onehotbatch(decision[i], 0:1)) for i in lower_test+1:upper_test]

    if lower_test == 1
        train_data = [(data_encoded[i,:], onehotbatch(decision[i], 0:1)) for i in upper_test+1:length(decision)]
    elseif upper_test == length(decision)
        train_data = [(data_encoded[i,:], onehotbatch(decision[i], 0:1)) for i in 1:lower_test]
    else
        train_data = [(data_encoded[i,:], onehotbatch(decision[i], 0:1)) for i in 1:lower_test]
        train_data = vcat(train_data, [(data_encoded[i,:], onehotbatch(decision[i], 0:1)) for i in upper_test+1:length(decision)])
    end
...
function create_perceptron(input_length)
    model = Flux.Dense(input_length => 2)
    model
end

# split training and test set (10-CV or 5-CV)
...
function train_perceptron(file::String, cv=10)
    device = cpu
    ...
    for cross in 1:cv
        ...
        for (x, y) in train_data
            x, y = device(x), device(y)
            gs = gradient(m -> loss(m(x), onecold(y)), model)
